...
class CrosshairView @JvmOverloads constructor(
    ...
    private fun drawCrosshair(canvas: Canvas, centerX: Float, centerY: Float) {
        val size = getScaledSize()
        val halfSize = size / 2
        val gap = size * 0.15f // Маленький зазор в центре крестика

        paint.style = Paint.Style.STROKE

        // Вертикальная линия
        canvas.drawLine(centerX, centerY - halfSize, centerX, centerY - gap, paint)
        canvas.drawLine(centerX, centerY + gap, centerX, centerY + halfSize, paint)

        // Горизонтальная линия
        canvas.drawLine(centerX - halfSize, centerY, centerX - gap, centerY, paint)
        canvas.drawLine(centerX + gap, centerY, centerX + halfSize, centerY, paint)
    ...
    private fun drawCircle(canvas: Canvas, centerX: Float, centerY: Float) {
        ...
        // Маленький центральный крест
        val crossSize = radius * 0.2f
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        // Для оверлея обычно используется фиксированный размер
