...
class CrosshairRepository(private val context: Context) {
    ...
    companion object {
        ...
        private const val APP_DARK_MODE_KEY = "app_dark_mode"
        private const val IGNORE_CUTOUT_KEY = "ignore_cutout"
    }
    ...
    /**
     * Тема оформления (SYSTEM / LIGHT / DARK / AMOLED)
     */
    fun getAppDarkModeFlow(): Flow<String> = dataStore.data.map { preferences ->
        preferences[stringPreferencesKey(APP_DARK_MODE_KEY)] ?: "SYSTEM"
    }

    suspend fun setAppDarkMode(mode: String) {
        dataStore.edit { preferences ->
            preferences[stringPreferencesKey(APP_DARK_MODE_KEY)] = mode
        }
    }

    /**
     * Игнорировать вырез камеры при позиционировании прицела
     */
    fun getIgnoreCutoutFlow(): Flow<Boolean> = dataStore.data.map { preferences ->
        preferences[booleanPreferencesKey(IGNORE_CUTOUT_KEY)] ?: true
    }

    suspend fun setIgnoreCutout(enabled: Boolean) {
        dataStore.edit { preferences ->
            preferences[booleanPreferencesKey(IGNORE_CUTOUT_KEY)] = enabled
        }
    }
    ...
    private fun serializeConfig(config: CrosshairConfig): String {
        return try {
            JSONObject().apply {
                ...
                put("showCenterCross", config.showCenterCross)
                put("centerCrossSize", config.centerCrossSize.toDouble())
    ...
    private fun deserializeConfig(json: String): CrosshairConfig {
        return try {
            ...
            CrosshairConfig(
                ...
                showCenterCross = obj.optBoolean("showCenterCross", true),
                centerCrossSize = obj.optDouble("centerCrossSize", 0.2).toFloat(),
    ...
    /**
     * Экспорт кастомных конфигов в JSON строку
     */
    fun exportConfigsJson(configs: List<CrosshairConfig>): String {
        return serializeConfigList(configs)
    }

    /**
     * Импорт конфигов из JSON строки — возвращает количество добавленных конфигов
     */
    suspend fun importConfigsFromJson(json: String): Int {
        val imported = deserializeConfigList(json).filter { !it.isPreset }
        if (imported.isEmpty()) return 0
        val current = getCustomConfigsList()
        val existingIds = current.map { it.id }.toSet()
        val newConfigs = imported
            .filter { it.id !in existingIds }
            .map { it.copy(id = java.util.UUID.randomUUID().toString()) }
        if (newConfigs.isEmpty()) return 0
        val merged = current + newConfigs
        dataStore.edit { preferences ->
            preferences[stringPreferencesKey(CUSTOM_CONFIGS_KEY)] = serializeConfigList(merged)
        }
